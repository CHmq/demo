...
import { SUPPORT_LOCALES, LOCATION } from "config/locale.json";
...
const locations = [...Object.keys(SUPPORT_LOCALES), "hk_GFW"];
...
class LanguageSwitch extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if(!!translations.fileName) {
        ...
        if (this.checkFileLoaded(translations.fileName , currentLocation.value , true)) {
          this.httptranslations({
  ...
  checkFileLoaded = (i_file = "" , i_locale = "zh" , i_retBoolean = false) => {
    ...
    let ret =_fileList.filter(_file => !(Object.keys(this.$$langCache[i_locale]).indexOf(_file) > -1));
...
//        });
  ...
  handleClick = ({ item, key }) => {
    ...
    switch (my_key) {
      ...
      case "locations":
        return key === route.currentLocation
          ? toast.createToast({
              msg: "您已經處在當前所選的地區啦"
            })
          : (window.location = `/${key}`);
      ...
      <Menu
        ...
        defaultSelectedKeys={[
          route.currentLanguage
            ...
            : SUPPORT_LOCALES[LOCATION].lang.url,
        ...
        >
          {locations.map(value => (
            <Menu.Item key={value} style={liStyle} my_key="locations">
              {translations.initDone &&
                intl.get(`home.publicMsg.header.languageList.locations.${value}`)}
            </Menu.Item>
          ))}
        </Menu.ItemGroup>
      </Menu>
