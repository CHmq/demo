/*
 * @使用方法: 创建组件<Memulist  memuData={父组件获取数据}>
 */
...
export default class Memulist extends Component {
  // 设置默认值
  static defaultProps = {
    picUrl: "", // 图片链接
    title: "", // 标题
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        {!!picUrl && !!title ? (
          <h5 className={list.itemtitle} style={styleCss.title}>
            {title}
          </h5>
        ) : (
          ""
        )}
        {date ? <span className={list.message}>{date}</span> : ""}
      </div>
    );
  }
}
