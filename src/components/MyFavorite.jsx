  ...
  render() {
    ...
    return (
      <div className='history_container' style={{ backgroundColor: "#fff" }}>
        <Row style={{padding: '64px 0', background: `url(${bgPic})`}} type='flex' justify='center'>
          <TitleCard 
            ...
            <Row gutter={40} style={{padding: '20px 45px', height: 420}}>
              {this.state.loading && this.state.listData.length ? 
                this.state.listData.map((item) => (
                  <Col key={item.id} lg={6} md={8} sm={12} xs={24} style={{marginBottom: 20}}>
                    <Adapter res_type={item.res_type} ref_id={item.res_id} id={item.id}>
                      <Memulist 
                      picUrl={item.file} 
                      title={item.name}
                      titleFontSize='12px'
