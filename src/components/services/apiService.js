...
let _successCallback = function (response) {
    if (typeof response.data !== 'undefined' && (!!('result' in response.data) && parseInt(response.data.result) === 1)) {
...
let _errorCallback = function (error) {
    ...
    if (error !== false && ["object", "array"].indexOf(typeof error) > -1 && !!('result' in error)) {
...
_client.uploadOSS = function (_permit, file) {
    return new Promise((resolve, reject) => {
        ...
        xhr.send(file);
    }).then(_successCallback).catch(_errorCallback);
}
