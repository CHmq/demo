...
import { Form, Button, Input, Row, Col, message , Divider } from "antd";
    ...
    return (
      <div>
        <Form {...formItemLayout} onSubmit={this.handleSubmit}>
          <Form.Item
            {...formItemLayout}
            label={translations && intl.get("loading.publicMsg.register.verify.code")}
          >
            <Row gutter={8}>
              <Col span={10}>
                {getFieldDecorator("captcha", {
                  rules: [
                    { required: true, message: translations && intl.get("loading.publicMsg.register.verify.codeMsg") },
                    {
                      pattern: /^\d{6}$/,
                      message: translations && intl.get("loading.publicMsg.register.verify.codeWarnMsg")
                    }
                  ]
                })(<Input />)}
              </Col>
              <Col span={12}>
                <Button onClick={this.sendCode} disabled={!this.state.liked}>
                  {this.state.liked ? (
                    ...
                    <span>
                      {this.state.seconds + " s " }{translations &&
                        intl.get("loading.publicMsg.register.verify.resendbtn")}
                    </span>
                  )}
                </Button>
              </Col>
          ...
          >
            <Button type="primary" htmlType="submit" disabled={this.vaIidate()} style={{marginRight : "0.5rem"}}>
              {translations && intl.get("loading.publicMsg.register.verify.btn")}
            </Button>
